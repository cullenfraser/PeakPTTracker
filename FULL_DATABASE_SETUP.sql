...
-- Add client and trainer links after table creation
...
-- ============================================
-- 8. HOURS TABLE (for time tracking)
-- ============================================
...
-- ============================================
-- 9. HELPER FUNCTIONS & TRIGGERS (Before RLS)
-- ============================================

-- Function to auto-create client from contract (only if columns exist)
DO $$
BEGIN
  -- Check if client_id and trainer_id columns exist before creating function
  IF EXISTS (
...
AND column_name IN ('session_type', 'class_type', 'team_name', 'attendance_data')
UNION ALL
SELECT 
...
BEGIN
  ...
  RAISE NOTICE '   - trainers (with calendar_color)';
  RAISE NOTICE '   - clients (with is_active)';
  RAISE NOTICE '   - quotes';
  RAISE NOTICE '   - contracts (with client_id, trainer_id)';
  RAISE NOTICE '   - training_sessions (with session_type, class_type, team_name)';
  RAISE NOTICE '   - client_trainer_assignments';
  RAISE NOTICE '   - hours';
  RAISE NOTICE '';
